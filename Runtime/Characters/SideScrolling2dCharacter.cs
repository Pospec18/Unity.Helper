...
{
    public abstract class SideScrolling2dCharacter : Character2D
    {
        [Header("Movement")]
        ...
        /// <summary>Is used as input fot the movement</summary>
        protected float HorizMovement { get; set; }
        ...
        {
            Move();
            OnGround = groundCheck.IsTouchingLayers(groundMask);
        ...
        {
            Vector2 vel = Rb.velocity;
            float normHorVel = Mathf.Clamp(HorizMovement, -1, 1);
        ...
        private IEnumerator Jump()
        {
            ...
            float t = 0;
            while (Jumping && t < jumpDuration && Rb.velocity.y > 0)
            {
                t += Time.deltaTime;
                yield return null;
            }

            if (t < jumpDuration && !Jumping)
            {
                vel = Rb.velocity;
